...
(async () => {
  ...
  const voiceResult = await run(
    voiceGenerator,
    `Please rewrite the following tweets based on the notes post.\n\n<note>\n${notesPost}\n</note>\n\n${initialResult.finalOutput}\n\nRespond only with <tweet>...</tweet> tags for the tweets in the thread. The first tweet should end with two emoji, one relevant to the thread + 👇 so people can click to read the rest of the thread. The last tweet should end with "Here's the original note from [year]: [link to markdown file]"`
  );

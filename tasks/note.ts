...
(async () => {
  ...
  const voiceResult = await run(
    voiceGenerator,
    `Please rewrite the following based on the note in 100-200 words.\n\n<note>\n${notesPost}\n</note>\n\n${initialResult.finalOutput}\n\nThe first paragraph should end with two emoji, one relevant to the thread + 👇 so people can click to read the rest of the thread. Include the link to the note at the end.`
  );
